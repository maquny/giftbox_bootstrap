			            ...
			            <swiper ref="mySwiper" :options="swiperOptions">
			                <swiper-slide v-for="item in items" :v-Key="item">
			                	<div class="sell_tbl">
				                	...
				                	<div class="sell_td"><span class="brand"><img :src="item.sellerBrand"></span> {{item.name}}<span>{{item.price}} 원</span></div>
				                	<div class="sell_td">{{item.priceRequest}}</div>
				                	<div class="sell_td">{{item.requestComplete}}</div>
				                	<div class="sell_td"><span>{{item.state}}</span></div>
			                	</div>
			                </swiper-slide>
			            </swiper>
		        	</div>
	...
	export default {
		...
		data() {
			return {
				...
				items: [
					{ day: '09.22', sellerBrand: require('@/assets/culture.jpg'), name: '홍*동', price: '100,000', priceRequest: '20.09.22', requestComplete: '1초', state: '처리중' },
					{ day: '09.22', sellerBrand: require('@/assets/happy.jpg'), name: '홍*동', price: '100,000', priceRequest: '20.09.22', requestComplete: '1초', state: '처리중' },
					{ day: '09.22', sellerBrand: require('@/assets/happy.jpg'), name: '홍*동', price: '100,000', priceRequest: '20.09.22', requestComplete: '1초', state: '처리중' },
					{ day: '09.22', sellerBrand: require('@/assets/happy.jpg'), name: '홍*동', price: '100,000', priceRequest: '20.09.22', requestComplete: '1초', state: '처리중' },
					{ day: '09.22', sellerBrand: require('@/assets/culture.jpg'), name: '홍*동', price: '100,000', priceRequest: '20.09.22', requestComplete: '1초', state: '처리중' },
					{ day: '09.22', sellerBrand: require('@/assets/happy.jpg'), name: '홍*동', price: '100,000', priceRequest: '20.09.22', requestComplete: '1초', state: '처리중' },
				...
				faqData: [
					{text: 'faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.', day: '2020.09.20'},
					{text: 'faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.', day: '2020.09.20'},
					{text: 'faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.', day: '2020.09.20'},
					{text: 'faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.', day: '2020.09.20'},
					{text: 'faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.', day: '2020.09.20'},
					{text: 'faq 제목을 표시하다가 말줄임이 되게 끔 만들어야 합니다.', day: '2020.09.20'},
...
body{font-size:13px;}
.container_box{}
.ma_title{padding:15px;border:solid 1px #ddd;box-shadow:0 3px 6px 0 rgba(0,0,0,0.06);color:#444;font-weight:700;}
.swiper_box{border:solid 1px #ddd;border-top:none;box-shadow:0 3px 6px 0 rgba(0,0,0,0.06);}
.swiper_container_box{height:291px;box-shadow:0 3px 6px 0 rgba(0,0,0,0.06);position:relative;}
...
.custom-control-label{padding:2px 0;font-size:13px;}
.calu_radio{}
...
.calu_moneysum .line{float:left;width:50%;text-align:left;line-height:34px;font-size:13px;}
.calu_moneysum .line:first-child + .line{text-align:right;}
.calu_moneysum .line .form-control{font-size:13px;text-align:right;}
.calu_tbl{}
.calu_tbl:after,
.calu_tbl .calu_tr:after{content:'';display:block;clear:both;}
.calu_tbl .calu_tr{background-color:#f3f3f3;padding:16px 20px;border-radius:7px;}
.calu_tbl .calu_td{float:left;width:100%;padding:10px 0;}
.calu_tbl .calu_td div{float:left;width:50%;text-align:left;font-size:13px;}
...
.sell_tbl{width:100%;font-size:12px;}
...
.sell_tbl .sell_td .brand img{height:100%;}
...
.faq_container .list{float:left;width:100%;position:relative;padding-right:90px;font-size:13px;line-height:30px}
